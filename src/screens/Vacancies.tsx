...
export const Vacancies: React.FC<VacanciesProps> = ({ activeRole }) => {
  ...
  const toggleMockMode = () => {
    ...
    if (next) {
      ...
      setQuestions(MOCK_QUESTIONS.map((q) => ({ ...q, candidateAnswer: MOCK_CANDIDATE_ANSWERS[q.id] ?? '' })));
    } else {
      resetVacancyState();
    }
  };
      ...
      {currentStep === 'interview' && activeRole === 'hr' && (
        <section>
          ...
          <div className="bg-surface-container-lowest p-5 rounded-2xl shadow-sm mb-6">
            ...
            <div className="flex gap-3">
              {approvedCandidates.map((c) => (
                <button key={c.id} onClick={() => handleSelectCandidate(c.id)} className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${selectedCandidate === c.id ? 'bg-secondary text-white shadow-md' : 'bg-surface-container-low text-on-surface-variant hover:bg-surface-container-high'}`}>
